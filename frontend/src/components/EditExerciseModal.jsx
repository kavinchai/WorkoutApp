  ...
  return (
    <Modal title={'Edit: ' + exerciseName} onClose={onClose}>
      <form className="wbm-form" onSubmit={submit}>
        ...
        {deletePhase === 'confirm' && (
          <div className="modal-actions" style={{ justifyContent: 'space-between' }}>
            <span style={{ fontSize: 'var(--fs-sm)', color: 'var(--danger)' }}>Delete this exercise?</span>
            <div style={{ display: 'flex', gap: 8 }}>
              <button type="button" className="btn-ghost" onClick={() => setDeletePhase(null)}>Cancel</button>
              <button type="button" className="btn btn-sm btn-danger" onClick={deleteExercise} disabled={saving}>Confirm Delete</button>
            </div>
          </div>
        )}
        {deletePhase === 'deleted' && (
